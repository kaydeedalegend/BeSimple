...
class BeSimpleSupabaseUser extends BaseAuthUser {
  BeSimpleSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> beSimpleSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BeSimpleSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
